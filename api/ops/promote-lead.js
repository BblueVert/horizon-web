...
module.exports = async function handler(req, res) {
  ...
  try {
    const lr = await httpsRequest('GET',
      `${SB_URL}/rest/v1/leads?id=eq.${encodeURIComponent(leadId)}&select=id,nombre,plan,status&limit=1`, auth);
    ...
    const pr = await httpsRequest('GET',
      `${SB_URL}/rest/v1/projects?lead_id=eq.${encodeURIComponent(leadId)}&select=id&limit=1`, auth);
    ...
    await httpsRequest('POST', `${SB_URL}/rest/v1/projects`, headers,
      { id: projectId, lead_id: leadId, nombre: lead.nombre||'Proyecto', plan: lead.plan||'', estado: 'init' });

    for (const nombre of (MILESTONES_BASE[lead.plan] || MILESTONES_BASE.plan01)) {
      await httpsRequest('POST', `${SB_URL}/rest/v1/project_milestones`, headers,
        { id: uid(), project_id: projectId, nombre, estado: 'pending', deliverables: [] });
    }

    await httpsRequest('PATCH', `${SB_URL}/rest/v1/leads?id=eq.${encodeURIComponent(leadId)}`,
      { ...auth, 'Content-Type': 'application/json', Prefer: 'return=minimal' }, { status: 'activo' });

    return res.status(201).json({ ok: true, project_id: projectId });
  } catch (err) {
    console.error('[ops/promote-lead]', err.message);
