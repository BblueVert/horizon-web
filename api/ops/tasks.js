...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    const r = await httpsRequest('GET',
      `${SB_URL}/rest/v1/tasks?project_id=eq.${encodeURIComponent(pid)}&order=orden.asc,created_at.asc`, auth);
  ...
  if (req.method === 'POST') {
    ...
    const task = {
      id: uid(), project_id: sanitize(b.project_id,40), titulo: sanitize(b.titulo,200),
  ...
  if (req.method === 'PATCH') {
    ...
    if (['pendiente','en_progreso','hecho'].includes(b.estado)) patch.estado=b.estado;
    if (['alta','media','baja'].includes(b.prioridad)) patch.prioridad=b.prioridad;
