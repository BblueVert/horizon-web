...
const { httpsRequest, httpsPost, sanitize, rateLimit, getIp } = require('./shared');
...
module.exports = async function handler(req, res) {
  ...
  // GET — todos los leads para el pipeline
  if (req.method === 'GET') {
    const r = await httpsRequest('GET',
      `${SB_URL}/rest/v1/leads?select=*&order=created_at.desc`,
      auth
    );
    return res.status(r.status).json(JSON.parse(r.body || '[]'));
  }

  // PATCH — actualizar campos de un lead (status, mrr, nota, etc.)
  if (req.method === 'PATCH') {
    ...
    const r = await httpsRequest('PATCH',
      `${SB_URL}/rest/v1/leads?id=eq.${encodeURIComponent(id)}`,
      headers, patch
    );
