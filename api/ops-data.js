...
const PLAN_PRICES = {
  plan01: 290000, plan02: 490000,
  plan03: 690000, plan04: 890000, plan05: null,
};
...
module.exports = async function handler(req, res) {
  ...
  try {
    // Todos los leads (para cupos + actividad + pipeline)
    ...
    // MRR: suma de retainers de leads con status='activo'
    const activosConMrr = leads.filter(l => l.status === 'activo' && l.mrr > 0);
    const mrr = {
      total: activosConMrr.reduce((s, l) => s + (l.mrr || 0), 0),
      count: activosConMrr.length,
    };

    // Pipeline CLP: leads en etapas abiertas × precio del plan
    const enPipeline = leads.filter(l => PIPELINE_STATUSES.includes(l.status));
    const pipelineTotal = enPipeline.reduce((s, l) => {
      const p = l.tipoprecio === 'fundador' ? PLAN_PRICES[l.plan] : null;
      return s + (p || 0);
    }, 0);
    const pipeline = { total: pipelineTotal, count: enPipeline.length };

    // Proyectos activos
    const proyectosR = await httpsRequest('GET',
      `${SB_URL}/rest/v1/projects?estado=neq.entregado&select=id`,
      auth
    );
    const proyectos = (JSON.parse(proyectosR.body || '[]')).length;

    // Reuniones próximas (7 días)
    const desde = new Date().toISOString();
    const hasta = new Date(Date.now() + 7 * 86400 * 1000).toISOString();
    const reunionesLeads = leads.filter(l =>
      l.reunion_fecha &&
      l.reunion_fecha >= desde &&
      l.reunion_fecha <= hasta
    ).sort((a, b) => new Date(a.reunion_fecha) - new Date(b.reunion_fecha));

    return res.json({ mrr, pipeline, proyectos, leads, reuniones: reunionesLeads });
  } catch (err) {
    console.error('[ops-data]', err.message);
